...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("application_log.txt"),  # Log to file
...
st.set_page_config(
    page_title="Seamless multi-lingual chat application",
...
def AES_decrypt(ciphertext, aes_key):
    try:
        ...
        #logging.info(f"AES decryption successful: {plaintext}")
        return plaintext
    ...
    with st.form("signup_form"):
        ...
        if st.form_submit_button("Signup"):
            if not name:
                ...
                st.error("Age ield cannot be empty.")
...
def main():
    st.title("Secure Man-In-The-Middle attack prevention chat application")
    ...
    if page == "Signup/Login":
        ...
        if session_state.get("logged_in"):
            ...
            if selected_user != "Select a user":
                ...
                #             st.chat_message("You", avatar="👩‍🎨").write(chat["message"])
                #         else:
                #             st.chat_message(
                    ...
                    for chat in previous_chats["chat_history"]:
                        if chat["sender"] == user_info["name"]:
                            try:
                                ...
                                logging.info(f"Message decrypted (sent by you): {message}")

                                st.chat_message("You", avatar="👩‍🎨").write(message)
                            except Exception as e:
                            ...
                            if verify(chat["message"], chat["signature"], public_key):
                                try:
                                    message = AES_decrypt(chat["message"], previous_chats["aes_key"])
                                    logging.info(f"Message decrypted (sent by {selected_user_info['name']}): {message}")
                                    st.chat_message(
                                        selected_user_info["name"], avatar="🤖"
                                    ).write(message)
                                except cryptography.exceptions.InvalidSignature:
                                    st.error("Invalid signature")
                                    logging.error(f"Invalid signature detected for message from {selected_user_info['name']}")
                            else:
                                st.error("Invalid signature")
                                logging.error(f"Signature verification failed for message from {selected_user_info['name']}")
                            
                message = st.chat_input("Say something")
                if message:
                    ciphertext = AES_encrypt(message, previous_chats["aes_key"])
                    ...
                    logging.info(f"Message sent by {user_info['name']}: {message}")
                    logging.info(f"Message encrypted: {ciphertext}")
        ...
        if session_state.get("logged_in"):
            ...
            if start_button_clicked:
                logging.info("Intrusion detection started")
                st.session_state.is_running = True
                process_csv_with_delay(X_1, model, st)

            if stop_button_clicked:
                logging.info("Intrusion detection stopped")
                st.session_state.is_running = False

    elif page == "Download Logs":
        # Create a download button for the log file
        st.subheader("Download Application Logs")
        
        log_file_path = "application_log.txt"
        ...
        if os.path.exists(log_file_path):
            ...
            # Display the log file content as a preview
            st.text_area("Log Preview:", log_data, height=300)
            ...
            st.error("Log file not found. Please ensure logging is enabled.")
    else:
